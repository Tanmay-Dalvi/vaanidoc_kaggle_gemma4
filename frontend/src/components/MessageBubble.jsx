...
const MessageBubble = ({ message, isAI, onReplay, isSpeaking }) => {
    ...
    <div className={`flex w-full mb-4 ${isAI ? 'justify-start' : 'justify-end'}`}>
      ...
      >
        ...
        <div className={`flex items-center mt-2 text-xs opacity-70 ${isAI ? 'justify-between' : 'justify-end'}`}>
          {isAI && (
            <button 
              onClick={() => onReplay(message.response)}
              className={`p-1.5 rounded-full hover:bg-gray-100 transition-colors ${isSpeaking ? 'text-primary animate-pulse' : 'text-gray-500'}`}
